...
int main(int argc, char **argv){
	...
	coordinate cord_joint_1 = coordinate(0.0,0.0,0.0);
	coordinate cord_joint_2 = coordinate(0.0,0.0,14.0);
	coordinate cord_joint_3 = coordinate(0.0,0.0,64.0);
	coordinate cord_joint_5 = coordinate(42.0,0.0,53.0);
	coordinate cord_joint_6 = coordinate(52.0,0.0,53.0);

	vector<coordinate> my_joints;

	my_joints.push_back(cord_joint_1);
	my_joints.push_back(cord_joint_2);
	my_joints.push_back(cord_joint_3);
	my_joints.push_back(cord_joint_5);
	my_joints.push_back(cord_joint_6);

	int i;
	double r, lambda;

	double link_lengths[4];

	link_lengths[0] = 14.0;							// link_1_2_length
	link_lengths[1] = 50.0;							// link_2_3_length
	link_lengths[2] = 43.4165;						// link_3_5_length
	link_lengths[3] = 10.0;							// link_5_end_length
	...
	while(ros::ok()){
		...
		X += joy_msg.get_axis(3) * 0.030;
		Y += joy_msg.get_axis(0) * 0.030;
		Z += joy_msg.get_axis(1) * 0.030;

		//cout << endl << "X: " << X << "  " << "Y: " << Y << "  " << "Z: " << Z << "  " << endl;

		coordinate new_end_point_pos = coordinate(52.0 + X, 0.0 + Y, 53.0 + Z);

		//cout << "The target is: x = " << new_end_point_pos.get_x() << "  y = " << new_end_point_pos.get_y() << "  z = " << new_end_point_pos.get_z() << endl << endl;
		...
		if(distance_from_beginning > REACH){
			...
			double distance_to_target = new_end_point_pos.calculate_dist(my_joints[4]);
		...
		double joint_5_angle = find_angle(my_joints[3],my_joints[2],my_joints[4]);
		double joint_3_angle = find_angle(my_joints[2],my_joints[1],my_joints[3]);
		double joint_2_angle = find_angle(my_joints[1],my_joints[0],my_joints[2]);
		...
		coordinate proj_joint_1 = coordinate(my_joints[4].get_x(),my_joints[4].get_y(),0);
		double joint_1_angle = angle_of_vectors(proj_joint_1,reference_coord);
		...
		joint_angles[1] = joint_2_angle - 180.0;
		joint_angles[2] = (joint_3_angle + 14.676352) - 90.0;
		...
		cout << joint_angles[0] << "  " << joint_angles[1] << "  " << joint_angles[2] << "  " << joint_angles[3] << "  " << joint_angles[4] << endl;

		//ROS_INFO("%s", angles.data.c_str());
